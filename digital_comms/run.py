...
class DigitalCommsWrapper(SectorModel):
    ...
    def before_model_run(self, data):
        ...
        # Load assets
        assets = {}
        assets['premises'] = read_shapefile(
            os.path.join(data_path, 'assets_layer5_premises.shp'))
        assets['distributions'] = read_shapefile(
            os.path.join(data_path, 'assets_layer4_distributions.shp'))
        assets['cabinets'] = read_shapefile(
            os.path.join(data_path, 'assets_layer3_cabinets.shp'))
        assets['exchanges'] = read_shapefile(
            os.path.join(data_path, 'assets_layer2_exchanges.shp'))
    ...
    def simulate(self, data):
        ...
        technology = data_handle.get_parameter('technology')
        policy = data_handle.get_parameter('policy')
