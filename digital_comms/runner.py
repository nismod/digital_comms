...
def read_csv(file):
    ...
    -------
    list_of_dicts
        Returns a list of dicts, with each line [an asset or link] in the .csv forming its own dict
...
def read_links():
    ...
    -------
    list_of_dicts
        Returns a list_of_dicts containing all links between Premises, Distribution Points, Cabinets and Exchanges.
...
def write_decisions(decisions, year, technology, policy):
    ...
    decisions_filename = os.path.join(RESULTS_DIRECTORY,  'decisions_{}_{}.csv'.format(technology, policy))
...
if __name__ == "__main__": # allow the module to be executed directly
    ...
    for scenario, technology, policy in itertools.product(scenarios, strategy_technologies, policies):

        logging.info("--")
        logging.info("Running: {}, {}, {}".format(scenario, technology, policy))
        logging.info("--")
        ...
        for year in TIMESTEPS:
            ...
            # write_pcd_results(system, year, pop_scenario, throughput_scenario, technology, policy, cost_by_pcd)
